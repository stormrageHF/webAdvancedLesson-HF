...
JSX 语法的转换过程：
- JSX 语法会被转换为 React.createElement 方法的调用
- 调用方法返回 VirtualDOM 对象
- React 再将 VirtualDOM 转换为真实 DOM 
- 最后将真实 DOM 显示到页面
...
在项目 tiny-react 中，我们会自己实现一个 TinyReact.createElement 方法来处理 jsx 语法生成 virtual DOM 对象；
但是 Babel 在处理 jsx 的时候默认已经使用 React 的 createElement 方法，所以要配置一下将这个默认设置修改过来；
